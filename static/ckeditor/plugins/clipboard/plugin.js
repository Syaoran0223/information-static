	...
	CKEDITOR.plugins.add( 'clipboard', {
		...
		init: function( editor ) {
			...
			editor.on( 'paste', function( evt ) {
				...
				if ( data.dataValue ) {
					if(data.type=='html'){
						var d = $.parseHTML(data.dataValue);
						$.each(d,function(i,obj){
							if($(this).length>1){
								$("img",$(this)).each(function(index, el){
									//console.log(i+":"+index);
									var src = $(this).attr("src"); //.replace(/\\/gi,"\/");
									var _this = this;
									$(this).attr("alt","");
									if(src.startsWith("file:///")||src.startsWith("http")){
										//$(this).attr("src",JSConvert.imageToBase64(src));
										if(typeof(JSConvert)!='undefined'){
											//el.src = JSConvert.imageToBase64(src);
											$(this).attr("src",JSConvert.imageToBase64(src));
										}
									}
								});
						...
						if(d.length==1)
							data.dataValue = d[0].outerHTML;
						else{
							var list = [];
							$.each(d,function(i,obj){
								if(obj.outerHTML==undefined)
									list[i]=obj.nodeValue;
								else
									list[i]=obj.outerHTML;
							});
							data.dataValue = list.join("");
						}
					}
