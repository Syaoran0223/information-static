...
const module_actions = {
    ...
    parse_edit_init_data: ({ state, actions }, data) => {
        ...
        if (data.has_sub) {
            data.sub_items = _.map(data.sub_items, (d)=> {
                ...
                if (actions.is_selector(d.qtype_id)) {
    ...
    on_verify_right({ state, actions }) {
        let api = `${ api_host }/api/quest/verify/right/${ state.edit.params.id }`
        if (state.edit.verify_saving) return
        state.edit.verify_saving = true
        PUT(api).then((res) => {
            notify_ok({
                title: '保存成功'
            })
            ...
            state.edit.verify_saving = false
        })
      }
}
